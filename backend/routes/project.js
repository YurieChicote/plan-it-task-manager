 ...
 *         description: Created
 */
// make new project (admin/pm only)
router.post('/', protect, managementOnly, async (req, res) => {
    const { name, description, deadline } = req.body;
    
    try {
        // Automatically save the creator's user identity to handle the project ownership specification notes
        const savedProject = await Project.create({
            name,
            description,
            deadline,
            owner: req.user.id // Saves current login user as owner
        });
